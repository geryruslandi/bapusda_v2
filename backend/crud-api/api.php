...
class TokenHelper{
    ...
    public function createToken($username,$password){
        ...
        $userDataSql = "select id,fullname,memberno,job_id,educationlevel_id,sex_id,address,kelas_id,jenispermohonan_id from members
                      where fullname = '$username' and memberno = '$password'";

        $query = mysqli_query($con,$userDataSql);
        if(mysqli_num_rows($query) == 1){
            //Fetch User Data
            $userData=mysqli_fetch_assoc($query);

            //Create Token Proccess
            $issuedAt   = time();
            $notBefore  = $issuedAt + 2;  //Adding 10 seconds
            $expire     = $notBefore + 300; // Adding 2 hours
            ...
            $objJwt= new JWT();
            $jwt['name']=$userData['fullname'];
            $jwt['token'] = $objJwt::encode(
    ...
    public function createTokenBarcode($barcode){

        $con=mysqli_connect("127.0.0.1","root","root","inlislite_v3");

        $userDataSql = "select id,fullname,memberno,job_id,educationlevel_id,sex_id,address,kelas_id,jenispermohonan_id from members
                      where  memberno = '$barcode'";
        ...
        if(mysqli_num_rows($query) == 1){
            ...
            header('Unauthorized',true,401);
            $data['status'] = 'Error';
            $data['message'] = 'Kode barcode Salah';
            echo json_encode($data);
        }
    }
}
...
class PdfHelper extends FPDF {
    ...
    {
        parent::__construct($orientation, $unit, array(100,120));
    }
    ...
    function Header()
    {
        ...
        // Title
        $this->Cell(0,0,'DINAS PERPUSTAKAAN',0,0,'C');
        $this->Ln(4);
        $this->Cell(0,0,'DAN ARSIP',0,0,'C');
        $this->Ln(4);
        $this->Cell(0,0,'KOTA TANJUNGPINANG',0,0,'C');
        $this->Line(0,25,100,25);
        $this->Ln(10);
    }
    function Body($nama = 'tes',$no_anggota = 'tes',$nama_buku = 'tes',$tanggal_pinjam = 'tes',$tanggal_kembali = 'tes'){
        $this->Cell(30,6,'Nama',0,0,'L');
        $this->Cell(10,6,':',0,0,'C');
        $this->Cell(50,6,$nama,0,0,'L');
        $this->Ln(6);

        $this->Cell(30,6,'No Anggota',0,0,'L');
        $this->Cell(10,6,':',0,0,'C');
        $this->Cell(50,6,$no_anggota,0,0,'L');
        $this->Ln(6);

        $this->Cell(30,6,'Nama Buku',0,0,'L');
        $this->Cell(10,6,':',0,0,'C');
        $this->Cell(50,6,$nama_buku,0,0,'L');
        $this->Ln(6);

        $this->Cell(30,6,'Tanggal Pinjam',0,0,'L');
        $this->Cell(10,6,':',0,0,'C');
        $this->Cell(50,6,$tanggal_pinjam,0,0,'L');
        $this->Ln(6);

        $this->Cell(30,6,'Tanggal Kembali',0,0,'L');
        $this->Cell(10,6,':',0,0,'C');
        $this->Cell(50,6,$tanggal_kembali,0,0,'L');
        $this->Ln(6);
...
if($_GET['type'] == 'login' ){
    ...
    $tokenObj = new TokenHelper();
    $tokenObj->createToken($username,$password);
}
elseif($_GET['type'] == 'loginBarcode'){
    //convert json to post
    $jsonpost = json_decode(file_get_contents('php://input'), true);
    if($jsonpost != "" || $jsonpost != null ){
        $_POST = $jsonpost;
    ...
    $tokenObj = new TokenHelper();
    $tokenObj->createTokenBarcode($barcode);
}
elseif($_GET['type'] == 'pinjam'){
    $jsonpost = json_decode(file_get_contents('php://input'), true);
    if($jsonpost != "" || $jsonpost != null ){
        $_POST = $jsonpost;
    }
    ...
    $loan_status = 'loan';
    $collection_id = $_POST['book_id'];
    $member_id = $userdata->id;
    $create_date = $loan_date;
